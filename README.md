...
0. Standards to uphold:
   ...
   - Document whether a program opens by setting `opens=Yes` in the `Ratings` template. A program is considered "open" if it launches the title screen or main menu. Dialog messages don't count as a program "opening". Write down all issues that you have noticed while playing a title under a `> [!WARNING]` heading. For more examples of headings that can be used, see the [mdbook-callouts documentation](https://crates.io/crates/mdbook-callouts). For consistency among game pages, do not use custom titles, like "problem" or "issues". Only use the headings listed above.
